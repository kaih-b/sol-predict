...
stats = (seed_df.groupby('model')['test_rmse'].agg(mean='mean', std='std', n='count').reset_index())

# Prep ordering to display models best -> worst
order = ['MLP_ext', 'MLP_base', 'RF']
label_map = {'RF': 'Random Forest (Tuned)', 'MLP_base': 'MLP (Base)','MLP_ext': 'MLP (Expanded)'}
stats['model'] = pd.Categorical(stats['model'], categories=order, ordered=True)
stats = stats.sort_values('model')

# Prep label map and values for matplotlib in same order
labels = [label_map[m] for m in stats['model'].astype(str)]
means = stats['mean'].to_numpy()
stds = stats['std'].to_numpy()

# Plot each model's RMSE w/ std
plt.figure(figsize=(8, 4))
bars = plt.bar(labels, means, yerr=stds, capsize=6)
plt.ylabel('Test RMSE ± Std')
plt.title('Model Comparison Across Seeds')
plt.ylim(0, max(means + stds) * 1.25)
# Add labels for each bar
for i, (mu, sd) in enumerate(zip(means, stds)):
    plt.text(i, mu + sd + (0.01 * max(means + stds)),
             f'{mu:.3f}\n(±{sd:.3f})',
             ha='center', va='bottom')
plt.tight_layout()
plt.savefig('exports/updated_model_comparison.png', dpi=300)
plt.close